...
bool fixed_temperature_to_string(char *buffer, size_t buffer_size, int16_t temperature_tenths, char unit)
{
	int32_t magnitude;
	int written;
	...
	written = snprintf(buffer, buffer_size, "%s%ld.%ld %c", temperature_tenths < 0 ? "-" : "", (long)(magnitude / 10), (long)(magnitude % 10), unit);
