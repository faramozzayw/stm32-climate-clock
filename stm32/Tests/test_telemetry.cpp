...
static void test_telemetry_round_trips_limits(void)
{
	uint8_t frame[telemetry::max_frame_size];
	uint16_t frame_length;
	device_DeviceMessage message = device_DeviceMessage_init_zero;

	TEST_ASSERT_TRUE(telemetry::encode_limits(
		100, 300, 300U, 650U, frame, frame_length));
	TEST_ASSERT_TRUE(payload_length(frame) <= 20U);
	TEST_ASSERT_TRUE(decode_frame(frame, frame_length, &message));
	TEST_ASSERT_TRUE(
		message.payload.telemetry.which_data ==
		device_DeviceTelemetry_limits_tag);

	const auto &limits = message.payload.telemetry.data.limits;
	TEST_ASSERT_TRUE(limits.min_temperature_tenths_celsius == 100);
	TEST_ASSERT_TRUE(limits.max_temperature_tenths_celsius == 300);
	TEST_ASSERT_EQUAL_UINT32(
		300U,
		limits.min_humidity_tenths_percent);
	TEST_ASSERT_EQUAL_UINT32(
		650U,
		limits.max_humidity_tenths_percent);
}
...
int main(void)
{
	...
	RUN_TEST(test_telemetry_round_trips_limits);
